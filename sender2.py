# PRE SVEGA MORA DA SE INSTALIRA TABULATE komandom:  pip install tabulate
...
# Define headers for log file
headers = ['File Name', 'Osiguranje', 'Status']
...
with open(log_file, 'w') as f:
    for pdf in pdfs:
        ...
        email_to = email_dict.get(osig, email_dict['default'])
        ...
        # Attach PDF file
        with open(os.path.join(src_folder, pdf), 'rb') as f_pdf:
            attach = MIMEApplication(f_pdf.read(), _subtype='pdf')
        ...
        # Log status
        log_entry = [pdf, osig, status]
        log_data.append(log_entry)

    # Write log file
    f.write(tabulate(log_data, headers=headers, tablefmt="fancy_grid"))
